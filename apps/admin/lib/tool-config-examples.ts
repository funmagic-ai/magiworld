...
export const TOOL_CONFIG_EXAMPLES: Record<ToolSlug, ToolConfigExample> = {
  ...
  'fig-me': {
    ...
    tips: [
      ...
      '  1. transform: Converts user photo to figurine style (uses OpenAI GPT-image-1)',
      '  2. 3d: Generates 3D model from transformed image (uses 3D SDK)',
      '',
      ...
      '  - systemPrompt: Custom prompt for the AI transformation',
      ...
      '3D step config:',
      '  - name: "3d" (required)',
      '  - provider: 3d_tripo or 3d_hunyuan',
      '  - modelParams: Parameters for 3D generation (format, resolution, etc.)',
      '',
      'Make sure these providers are configured:',
      '  - openai: Required for transform step (uses Responses API with gpt-image-1.5)',
      '  - 3d_tripo or 3d_hunyuan: Required for 3D generation',
    ],
    exampleConfig: {
      steps: [
        {
          ...
          systemPrompt: 'Transform this photo into a cute 3D figurine style character. Make it colorful, toy-like, with smooth surfaces suitable for 3D printing. Keep the person\'s distinctive features but stylize them in a chibi/kawaii aesthetic.',
          size: '1024x1024',
          quality: 'high',
          format: 'png',
