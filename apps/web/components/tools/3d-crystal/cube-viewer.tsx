...
const fontLoadingPromises: Map<string, Promise<Font>> = new Map();

// Convert opentype path to Three.js font format (for English text fallback)
function convertPathCommands(path: opentype.Path, scale: number): string {
...
// Convert opentype glyph path to THREE.Shape (for Chinese text - better quality)
function convertGlyphToShapes(glyph: opentype.Glyph, scale: number): THREE.Shape[] {
  const path = glyph.getPath(0, 0, 1);
  const shapes: THREE.Shape[] = [];
  let currentShape: THREE.Shape | null = null;
  let currentPath: THREE.Path | null = null;
  const allPaths: { path: THREE.Path; clockwise: boolean }[] = [];
  ...
  for (const cmd of path.commands) {
    switch (cmd.type) {
      case 'M':
        if (currentPath) {
          // Calculate if path is clockwise (for hole detection)
          ...
          allPaths.push({ path: currentPath, clockwise: area < 0 });
        }
        currentPath = new THREE.Path();
        currentPath.moveTo(cmd.x * scale, cmd.y * scale);
        break;
      case 'L':
        currentPath?.lineTo(cmd.x * scale, cmd.y * scale);
        break;
      case 'C':
  ...
  // Second pass: create shapes with holes
  // In opentype.js: clockwise paths are outer shapes, counter-clockwise are holes
  for (const { path: p, clockwise } of allPaths) {
    if (clockwise) {
      // This is an outer shape
      currentShape = new THREE.Shape(p.getPoints());
      shapes.push(currentShape);
    } else if (currentShape) {
      // This is a hole in the current shape
      currentShape.holes.push(p);
    }
  }
...
// Cache for loaded opentype fonts
const opentypeFontCache: Map<string, opentype.Font> = new Map();
const opentypeFontPromises: Map<string, Promise<opentype.Font>> = new Map();

// Load opentype font directly
async function loadOpentypeFont(fontUrl: string): Promise<opentype.Font> {
  if (opentypeFontCache.has(fontUrl)) {
    return opentypeFontCache.get(fontUrl)!;
  }

  if (opentypeFontPromises.has(fontUrl)) {
    return opentypeFontPromises.get(fontUrl)!;
  }
  ...
  const promise = (async () => {
    ...
    if (!font) throw new Error('Failed to parse font');
    opentypeFontCache.set(fontUrl, font);
    return font;
  })();

  opentypeFontPromises.set(fontUrl, promise);
  return promise;
}

// Create 3D geometry for Chinese text using ExtrudeGeometry
function createChineseTextGeometry(
  text: string,
  ...
  console.log('Creating Chinese text geometry:', text, 'fontSize:', fontSize, 'scale:', scale);

  for (const char of text) {
    const glyph = opentypeFont.charToGlyph(char);
    if (!glyph || glyph.index === 0) {
      console.warn('Glyph not found for:', char);
      continue;
    }

    const shapes = convertGlyphToShapes(glyph, scale);
    console.log('Shapes for', char, ':', shapes.length);
    ...
    if (shapes.length > 0) {
      ...
      const charGeometry = new THREE.ExtrudeGeometry(shapes, extrudeSettings);
      charGeometry.translate(xOffset, 0, 0);
  ...
  if (geometries.length === 0) {
    console.error('No geometries created for Chinese text');
  ...
  if (mergedGeometry.boundingBox) {
    const centerX = (mergedGeometry.boundingBox.max.x + mergedGeometry.boundingBox.min.x) / 2;
    const centerY = (mergedGeometry.boundingBox.max.y + mergedGeometry.boundingBox.min.y) / 2;
    mergedGeometry.translate(-centerX, -centerY, -depth / 2);
  }

  return mergedGeometry;
}

// Simple buffer geometry merge function
...
// Import FontData type from Three.js
import type { FontData } from 'three/examples/jsm/loaders/FontLoader.js';

// Convert TTF font to Three.js format for specific characters
async function convertTTFToThreeJS(
  ...
  const allChars = [...new Set(defaultChars + charactersToInclude)].join('');
  ...
  for (const char of allChars) {
    ...
    glyphs[char] = {
      ...
      o: convertPathCommands(path, scale),
    };
  }
...
// Load Chinese font from various sources with fallback
async function loadChineseFont(textToRender: string): Promise<Font> {
  const cacheKey = 'chinese-' + textToRender;
  ...
  const promise = (async () => {
    // Try multiple font sources with fallbacks
    const fontUrls = [
      // Local font in public/fonts/
    ...
    for (const fontUrl of fontUrls) {
      try {
        ...
        const fontData = await convertTTFToThreeJS(fontBuffer, textToRender);
    ...
    console.error('All Chinese font sources failed, falling back to default');
    return loadDefaultFont();
  })();

  fontLoadingPromises.set(cacheKey, promise);
  return promise;
}

// Load default English font
async function loadDefaultFont(): Promise<Font> {
...
async function loadFont(text: string): Promise<Font> {
  if (containsChinese(text)) {
    return loadChineseFont(text);
  }
  return loadDefaultFont();
}
